...
def treinar_modelo(df, model_name):
  ...
  clf = Pipeline(
    steps=[
      ("prep", pre),("model", DecisionTreeClassifier(random_state=RANDOM_STATE, class_weight=CLASS_WEIGHT, max_depth=15, min_samples_leaf=50)),
    ]
  )
  ...
  try:
    if hasattr(clf.named_steps["model"], "feature_importances_"):
      ...
      imp_df = (pd.DataFrame({"feature": feature_names, "importance_pct": importances * 100}).sort_values("importance_pct", ascending=False))
  ...
  # ===== Dicionário de métricas para comparação =====
  metrics = {
    "model": model_name,
